...
public abstract class AbstractUploader {
    ...
    static DecimalFormat decimalFormat = new DecimalFormat("#.00", java.text.DecimalFormatSymbols.getInstance(java.util.Locale.US));

    public static void printStatus(float s) {
        System.out.println("Progress: " + decimalFormat.format(s*100) + "%");
        System.out.flush();
        return;
    }
    ...
    public void parseArgs(String[] args) {
        ...
        for (String arg : args) {
            ...
            if (arg.equalsIgnoreCase("-listonly")) {
                ...
                manifestPath = arg.substring(arg.indexOf(argSeparator) + 1);
                println("Using manifest file: " + manifestPath);
...
System.out.println("tagId: " + tagId);
    ...
    public static class ManifestDirectory extends Resource {
    ...
    private void uploadManifest(String manifestPath) throws Exception {
        byte[] encoded = java.nio.file.Files.readAllBytes(java.nio.file.Paths.get(manifestPath));
        String content = new String(encoded, java.nio.charset.StandardCharsets.UTF_8);
        JSONArray arr = new JSONArray(content);
        
        ManifestDirectory rootDir = new ManifestDirectory("");
        
        for (int i = 0; i < arr.length(); i++) {
            JSONObject obj = arr.getJSONObject(i);
            String source = obj.getString("source");
            String directoryLabel = obj.optString("directoryLabel", "");
            
            // Normalize directoryLabel: replace backslashes, strip slashes
            directoryLabel = directoryLabel.replace("\\", "/").trim();
            ...
            FileResource fileRes = new FileResource(source);
            
            if (directoryLabel.isEmpty()) {
                rootDir.addResource(fileRes);
            } else {
                String[] parts = directoryLabel.split("/");
                ManifestDirectory current = rootDir;
                for (String part : parts) {
                    if (part.isEmpty()) continue;
                    ManifestDirectory child = null;
                    for (Resource r : current.listResources()) {
                        if (r.isDirectory() && r.getName().equals(part)) {
                            child = (ManifestDirectory) r;
                            break;
                        }
                    }
                    if (child == null) {
                        child = new ManifestDirectory(part);
                        current.addResource(child);
                    }
                    current = child;
                }
                current.addResource(fileRes);
            }
        }
        
        // We call uploadCollection on rootDir
        String newUri = uploadCollection(rootDir, "", null, null);
        if (!listonly) {
            if (newUri == null) {
                postProcessCollection();
            } else {
                postProcessChildren(rootDir);
            }
        }
    }
